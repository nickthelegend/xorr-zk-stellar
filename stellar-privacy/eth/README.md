# ShieldedBridge — Ethereum side (Sepolia)
...
The frontend computes `commitment = Poseidon(amount, Poseidon(sk), blinding)`
(same as `frontend/src/lib/notes.ts`), encodes it big-endian into `bytes32`, then
calls `lock(amount, commitment)` via MetaMask. The returned `nonce` is what the
relayer references on the Stellar side.
