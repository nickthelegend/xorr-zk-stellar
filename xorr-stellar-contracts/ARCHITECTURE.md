...
```mermaid
flowchart TB
  subgraph Client["Off-chain — Client / Prover (this repo: circuits/)"]
    NOTES["Note manager + Poseidon Merkle tree<br/>(circomlibjs, BN254)"]
  ...
  subgraph Stellar["Stellar / Soroban (this repo: contracts/)"]
  ...
  subgraph Ethereum["Ethereum / Sepolia (this repo: eth/)"]
...
Nethermind's design. The `disclose.circom` circuit proves ownership and amount of
an on-chain note to an auditor without revealing the spend key or blinding. The
contract layer is unaffected; disclosure is a client capability.
